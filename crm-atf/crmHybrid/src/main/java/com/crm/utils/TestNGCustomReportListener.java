	...
	public class TestNGCustomReportListener implements IReporter {
		...
		private String reportTitle= "CRM Automation Report";
		private String reportFileName = "crm-automation-report.html";
		...
		private void resultSummary(ISuite suite, IResultMap tests, String testname,
			...
			if (tests.getAllResults().size() > 0) {
				...
				for (ITestNGMethod method : getMethodSet(tests, suite)) {
					...
					buff.append("<td><a href=\"#m"+m_methodIndex+"\">"+qualifiedName(method)+" "+description+"</a> <br/> "+(testInstanceName != null && testInstanceName.length()>0 ? testInstanceName : "" )+" </td>" +
...
//					writer.println("</td>");
		...
		private void tableStart(String cssclass, String id) {
			writer.println("<table cellspacing=\"0\" cellpadding=\"0\"" + (cssclass != null ? " class=\"" + cssclass + "\"" : "style=\"padding-bottom:2em\"")
							+ (id != null ? " id=\"" + id + "\"" : "") + ">");
			m_row = 0;
		}
		...
		protected void startHtml(PrintWriter out) {
			...
			out.println("<html>");
			out.println("<head>");
			out.println("<title>CRM Automation Report</title>");
		...
		protected void endHtml(PrintWriter out) {
			out.println("<center> PHWeb Automation Report </center>");
